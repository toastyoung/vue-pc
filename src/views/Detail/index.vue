    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :imgSrc="skuInfo.skuDefaultImg" />
          <!-- 小图列表 -->
          <ImageList :imgSrc="skuInfo.skuDefaultImg" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
...
export default {
  ...
  async mounted() {
    const data = await reqGetGoodsDetail(this.$route.params.id);
    this.categoryView = data.categoryView;
    this.skuInfo = data.skuInfo;
    this.spuSaleAttrList = data.spuSaleAttrList;
  },
  methods: {
    setIsChecked(spuSaleAttrValue, spuSaleAttrValueList) {
